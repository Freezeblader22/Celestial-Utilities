...
public class ResearchTableTile extends TileEntity {

    private final ItemStackHandler itemHandler = createHandler();
    private final LazyOptional<IItemHandler> handler = LazyOptional.of(() -> itemHandler);

    public ResearchTableTile(TileEntityType<?> tileEntityTypeIn) {
        super(tileEntityTypeIn);
    }

    public ResearchTableTile() {
        this(ModTileEntities.RESEARCH_TABLE_TILE.get());
    }
        ...
        return new ItemStackHandler(2) {
            ...
            public boolean isItemValid(int slot, @Nonnull ItemStack stack) {
                   switch (slot) {
                       ...
                       case 1: return stack.getItem() == Items.FIRE_CHARGE || stack.getItem() == Items.STICK;
    ...
    public void lightningHasStruck() {
        ...
        boolean hasAmethystInSecondSlot = this.itemHandler.getStackInSlot(1).getCount() > 0
                && this.itemHandler.getStackInSlot(1).getItem() == Items.FIRE_CHARGE;

        if(hasFocusInFirstSlot && hasAmethystInSecondSlot) {
            this.itemHandler.getStackInSlot(0).shrink(1);
            this.itemHandler.getStackInSlot(1).shrink(1);

            this.itemHandler.insertItem(1, new ItemStack(Items.STICK), false);
        }
    }
